...
export const useCartesiaTTS = ({ onStart, onEnd, onError }: UseCartesiaTTSProps = {}) => {
    ...
    useEffect(() => {
        const initCartesia = async () => {
            try {
                ...
                const apiKey = localStorage.getItem("cartesia-api-key") || process.env.NEXT_PUBLIC_CARTESIA_API_KEY;

                if (!apiKey) {
                    console.warn("⚠️ Cartesia API key not found. Using browser TTS fallback.");
