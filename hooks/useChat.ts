...
export const useChat = ({ initialChatData }: UseChatProps) => {
  const initialState = useMemo(() => {
    ...
    const sanitizeResumeData = (data: any): ResumeData => {
      ...
      const sanitized = {
        ...
        contact: data.contact && typeof data.contact === 'object' ? {
          ...
          blogs: typeof data.contact.blogs === 'string' ? data.contact.blogs : '',
        ...
        experience: Array.isArray(data.experience) ? data.experience.map((exp: any) => ({
          ...
          description: typeof exp?.description === 'string' ? exp.description :
            exp?.description ? JSON.stringify(exp.description) : '',
        ...
        projects: Array.isArray(data.projects) ? data.projects.map((proj: any) => ({
          ...
          description: typeof proj?.description === 'string' ? proj.description :
            proj?.description ? JSON.stringify(proj.description) : '',
          techStack: Array.isArray(proj?.techStack) ? proj.techStack.map((tech: any) =>
            typeof tech === 'string' ? tech : String(tech)
          ) : [],
        })) : EMPTY_RESUME.projects,
        achievements: Array.isArray(data.achievements) ? data.achievements.map((ach: any) =>
    ...
    if (Array.isArray(chatMessages)) {
      ...
      const invalidMessages = flatMessages.filter(msg => !isValidChatMessage(msg));

      if (invalidMessages.length > 0) {
        console.warn('Found invalid message structures:', invalidMessages);
        console.warn('These messages will be filtered out to prevent rendering errors');
      }
