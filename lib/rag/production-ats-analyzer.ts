...
const STOP_WORDS = new Set([
  ...
  // Common actions (that aren't technical skills)
  "work",
  "working",
  "develop",
  "build",
...
export class ProductionATSAnalyzer {
   ...
   */
  private extractTechnicalKeywords(
    text: string,
  ...
  private calculateSemanticSimilarity(
    ...
    const resumeWords = new Set(
      resumeNorm.split(" ").filter((w) => w.length > 2),
    );
    ...
    const embeddingSimilarity =
      this.cosineSimilarity(resumeEmbedding, jobEmbedding) * 100;
  ...
  private extractRoleLevel(text: string): number {
    ...
    for (let level = 4; level >= 0; level--) {
      if (
        levelKeywords[level].some((keyword) => normalizedText.includes(keyword))
      ) {
        return level;
      }
    }
  ...
  analyze(resumeText: string, jobDescription: string): ProductionATSAnalysis {
    ...
    const skillMatchRatio =
      matchedSkillsCount / (matchedSkillsCount + missingSkillsCount);
  ...
  private generateRecommendations(
    ...
    if (missingSkills.length > 0) {
      recommendations.push(
        `Add these critical skills: ${missingSkills.slice(0, 3).join(", ")}`,
      );
    }

    if (formatQuality < 70) {
      recommendations.push(
        "Improve resume structure with clear sections (Summary, Experience, Skills, Education)",
      );
    }

    if (keywordDensity < 40) {
      recommendations.push(
        "Include more relevant technical keywords naturally throughout your resume",
      );
    }

    if (experienceMatch < 60) {
      recommendations.push(
        "Highlight projects or experiences that demonstrate the required skill level",
      );
    }

    if (recommendations.length === 0) {
      recommendations.push(
        "Your resume looks strong! Consider minor optimizations for specific job requirements.",
      );
    }
